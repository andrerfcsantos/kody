...
var restoreCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		matches, err := filepath.Glob(filepath.Join(outputDir, w.Slug(), sectionGlob, exerciseGlob))
		if err != nil {
			return fmt.Errorf("globbing files: %w", err)
		}

		if len(matches) == 0 {
			return errors.New("no candidate directory in the output directory found for the exercise was found")
		}
		...
		err = os.CopyFS(w.PlaygroundPath(), os.DirFS(restorePath))
		if err != nil {
